...
summarize.surv.sign <- function(tissues, project,
    ...
    for (tissue in tissues){               
        ...
        for (sex in sexes){  ## TCGA survival subset (all, male, female)
            ...
            DEGenes.tmp <- rbind(DEGenes.tmp,
                                 cbind(project, tissue, sex, desex="either",
                                       ps[[sex]][gene.ind,],
            ...
            ## DE IN MALES ##
            DEGenes.tmp <- rbind(DEGenes.tmp,
                                 cbind(project, tissue, sex, desex="male",
                                       ps[[sex]][gene.ind.m,],
            ...
            ## DE IN FEMALES
            DEGenes.tmp <- rbind(DEGenes.tmp,
                                 cbind(project, tissue, sex, desex="female",
                                       ps[[sex]][gene.ind.f,],
            ...
            DEGenes.tmp <- semi_join(DEGenes.tmp, logPs[,c("SYMBOL", tissue)], by = "SYMBOL")
            names(DEGenes.tmp)[grep(tissue , names(DEGenes.tmp))] <-  "logP"
            DEGenes.tmp <- semi_join(DEGenes.tmp, logFC[,c("SYMBOL", tissue)], by = "SYMBOL")
            names(DEGenes.tmp)[grep(tissue, names(DEGenes.tmp))] <- "logFC"
    ...
    ##names(survPs) <- c("project", "tissue", "SurvSex", "URsex", "p.mean",
    ##                   "p.med","p.min", "q.mean", "q.med", "q.min", "empP.p.mean",
    ##                   "empP.p.med","empP.p.min", "empP.q.mean", "empP.q.med", "empP.q.min")
    ##names(DEGenes) <- c("project", "tissue", "SurvSex", "URsex", "ENSEMBL",
    ##                    "SYMBOL","chr","start","biotype", "p.surv")
...
sim.surv.summary <- function(P, Q, project, size, obs, niter=100, ci=0.025){
